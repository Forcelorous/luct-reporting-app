...
import { getAllCourses, getAllAssignments, getLecturers, assignLecturer, deleteAssignment } from '../../services/api';
...
export default function AssignLecturers() {
  ...
  const loadAll = async () => {
    try {
      const [coursesData, assignmentsData, lecturersData] = await Promise.all([
        getAllCourses(),
        getAllAssignments(),
        getLecturers(),
      ]);
  ...
  const handleAssign = async () => {
    if (!selectedLecturer) { Alert.alert('Error', 'Please select a lecturer.'); return; }
    if (!courseCode.trim()) { Alert.alert('Error', 'Please select or enter a course code.'); return; }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.card}>
          ...
          <View>
            ...
            <View style={styles.chips}>
              {courses.map(c => (
                <TouchableOpacity
                  key={c.id}
                  style={[styles.chip, courseCode === c.courseCode && styles.chipSelected]}
                  onPress={() => setCourseCode(c.courseCode)}
                >
                  <Text style={[styles.chipText, courseCode === c.courseCode && styles.chipTextSelected]}>
                    {c.courseCode}
                  </Text>
                </TouchableOpacity>
              ))}
        ...
        <TextInput style={styles.input} placeholder="Search lecturer by name or email..." value={search} onChangeText={setSearch} autoCapitalize="none" />
          ...
          <View style={styles.dropdownList}>
            {filteredLecturers.length === 0 ? (
              ...
              filteredLecturers.map(l => (
                <TouchableOpacity key={l.id} style={styles.dropdownItem} onPress={() => { setSelectedLecturer(l); setSearch(''); }}>
                  <View style={styles.dropdownAvatar}>
                    <Text style={styles.dropdownAvatarText}>{l.name?.charAt(0).toUpperCase()}</Text>
                  </View>
                  <View>
        ...
        <TouchableOpacity style={[styles.addBtn, adding && styles.addBtnDisabled]} onPress={handleAssign} disabled={adding}>
          {adding ? <ActivityIndicator color="#fff" /> : <Text style={styles.addBtnText}>ASSIGN LECTURER</Text>}
        </TouchableOpacity>
      </View>
            ...
            renderItem={({ item }) => (
              <View style={styles.assignCard}>
                <View style={styles.assignInfo}>
                  <Text style={styles.assignName}>{item.lecturerName || item.lecturerEmail}</Text>
                  <Text style={styles.assignEmail}>{item.lecturerEmail}</Text>
                  <View style={styles.assignMeta}>
                    <View style={styles.badge}><Text style={styles.badgeText}>{item.courseCode}</Text></View>
                    <Text style={styles.assignDate}>{item.assignedAt ? new Date(item.assignedAt).toDateString() : '—'}</Text>
                  </View>
                </View>
                <TouchableOpacity style={styles.deleteBtn} onPress={() => handleDelete(item.id, item.lecturerName || item.lecturerEmail, item.courseCode)}>
...
const styles = StyleSheet.create({
  ...
  chip: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4, backgroundColor: '#f9fafb' },
  ...
  selectedCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#eff6ff', borderRadius: 8, padding: 10, marginBottom: 10, borderWidth: 1, borderColor: '#1a56db' },
  selectedInfo: { flex: 1 },
  selectedName: { fontSize: 14, fontWeight: '700', color: '#1a56db' },
  selectedEmail: { fontSize: 12, color: '#6b7280', marginTop: 2 },
  dropdownList: { borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 8, backgroundColor: '#fff', marginBottom: 10, maxHeight: 180 },
  dropdownItem: { flexDirection: 'row', alignItems: 'center', padding: 10, borderBottomWidth: 1, borderBottomColor: '#f3f4f6' },
  dropdownAvatar: { width: 34, height: 34, borderRadius: 17, backgroundColor: '#6487d1', justifyContent: 'center', alignItems: 'center', marginRight: 10 },
  ...
  addBtn: { backgroundColor: '#00ff26', borderRadius: 8, padding: 12, alignItems: 'center', marginTop: 4 },
  addBtnDisabled: { backgroundColor: '#93c5fd' },
  addBtnText: { color: '#fff', fontWeight: '700', fontSize: 14 },
  sectionTitle: { fontSize: 15, fontWeight: '700', color: '#374151', marginBottom: 10 },
  assignCard: { backgroundColor: '#fff', borderRadius: 10, padding: 14, marginBottom: 8, flexDirection: 'row', alignItems: 'center', elevation: 1 },
  assignInfo: { flex: 1 },
